        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.services.update", [ $service->id ] ) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="get-slug  form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $service->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="get-slug form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $service->slug) }}" >
                    @if($errors->has('slug'))
                ...
                <div class="form-group">
                    ...
                    <textarea name="meta_description" style="min-height: auto" id="meta_description" rows="2" class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}">{{ old('meta_description', $service->meta_description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="ckeditor form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" rows="10" id="editor" name="body">{{ old('body', $service->body) }}</textarea>
                    @if($errors->has('body'))
                        <div class="invalid-feedback">
                            {{ $errors->first('body') }}
                        </div>
